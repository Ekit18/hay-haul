...
import { DeliveriesFilterFormValues, deliveriesSortKeys, deliveriesSortKeyToLabelMap } from './validation';
...
export type DeliveriesSortSelectInputProps = {
  containerClassName?: string;
};
export function DeliveriesSortSelect({ containerClassName }: DeliveriesSortSelectInputProps) {
  const { setValue, watch } = useFormContext<DeliveriesFilterFormValues>();

  const sortKey = watch('innerSortKey');

  const onSortKeyChange = (value: string) => {
    setValue('innerSortKey', value as DeliveriesFilterFormValues['innerSortKey'], {
      shouldDirty: true,
      shouldTouch: true,
      shouldValidate: true
    });
      ...
      <div className="w-full">
        <FormItem className="w-full">
          ...
          <Select onValueChange={onSortKeyChange} value={sortKey}>
            ...
            <SelectContent>
              <SelectGroup>
                {deliveriesSortKeys?.map((item) => (
                  <SelectItem key={item} value={item}>
                    {deliveriesSortKeyToLabelMap[item]}
                  </SelectItem>
                ))}
              </SelectGroup>
            </SelectContent>
